...
#fake_display = xvfbwrapper.Xvfb(width=1200, height=1000, colordepth=24)
#fake_display.start()
# ~~~~scripts_dir~~~~~~~~~~~ #
script_dir = os.path.dirname(os.path.realpath(__file__))
igv_directory = script_dir + '/bin/IGV_2.4.1'
# ~~~~ CUSTOM FUNCTIONS ~~~~~~ #
...
def make_snapshot_filename(region, height, suffix = None):
    ...
    if len(region) >= 4:
        ...
        return('{}.png'.format(name)) # '{}.png'.format(name) # don't include file extension! user must do this ahead of time!
    elif len(region) == 3:
...
def run_IGV_script(igv_script, igv_jar, memMB):
    ...
    #fake_display = xvfbwrapper.Xvfb(width=1280, height=740, colordepth=24)
    #fake_display.start()
    os.system(igv_command)
...
def main(input_files, region_file = 'regions.bed', genome = 'hg19', image_height = '500', outdir = 'IGV_Snapshots', igv_jar_bin = "bin/IGV_2.3.81/igv.jar", igv_mem = "4000", no_snap = False, suffix = None, nf4_mode = False, onlysnap = False):
    ...
    if no_snap == False:
        run_IGV_script(igv_script = batchscript_file, igv_jar = igv_jar_bin, memMB = igv_mem)
igv_jar = os.path.dirname(os.path.realpath(__file__))+'/bin/IGV_2.3.81/igv.jar'
if not os.path.isfile(igv_jar):
    print('WARNING: Can not find igv.jar under the bin/IGV_2.3.81 direction! You shoud provide the igv.jar path with arguments!')
    sys.exit()
...
def run():
    ...
    # optional flags
    parser.add_argument("-g", default = 'hg19', type = str, dest = 'genome', metavar = 'genome', help=" provide Name of the reference genome, Defaults to hg19")
    parser.add_argument("-ht", default = '600', type = str, dest = 'image_height', metavar = 'image height', help="Height for the IGV tracks")
    ...
    parser.add_argument("-bin", default = igv_jar, type = str, dest = 'igv_jar_bin', metavar = 'IGV bin path', help="Path to the IGV jar binary to run")
    parser.add_argument("-mem", default = "20000", type = str, dest = 'igv_mem', metavar = 'IGV memory (MB)', help="Amount of memory to allocate to IGV, in Megabytes (MB)")
