...
# === pp5 evidence file ====
    try:
        fh = open(paras['pp5_aa'], 'r')
        strs = fh.read()
        for line2 in strs.split('\n'):
            cls2 = line2.split('\t')
            if len(cls2) > 1:
                keys = cls2[0]+"_"+cls2[1]+"_"+cls2[2]+"_"+cls2[5]+"_"+cls2[6]
                keys = re.sub("[Cc][Hh][Rr]", "", keys)
                PP5_aachange_dict[keys] = cls2[3]
    except IOError:
        print("Error: can\'t read the  amino acid change file %s" % paras['pp5_aa'])
        print("Error: Please check it!")
        sys.exit()
    else:
        fh.close()

# ====PS3 biochemistry evidence file ====
    try:
        fh = open(paras['ps3_aa'], 'r')
        lines = fh.readlines()
        for line2 in lines:
            cls2 = line2.split('\t')
            cls2 = [x.strip() for x in cls2]
            keys = cls2[0] + "_" + cls2[1]
            PS3_aachange_dict[keys] = cls2[2]
    except IOError:
        print("Error: can\'t find the PS3 evidence file")
        sys.exit()
    else:
        fh.close()
...
def check_downdb():
    ...
    if not isExists:
        paras['database_locat'] = os.path.dirname(os.path.realpath(__file__))+'/annovar/humandb2'
    else:
    ...
    if not os.path.isfile(paras['annotate_variation']):
        paras['annotate_variation'] = os.path.join(os.path.dirname(os.path.realpath(__file__))+'/annovar', 'annotate_variation.pl')
    for dbs in ds.split():
...
def check_input():
    ...
    if inputft.lower() == 'vcf':
        if not os.path.isfile(paras['convert2annovar']):
            paras['convert2annovar'] = os.path.join(os.path.dirname(os.path.realpath(__file__))+'/annovar', 'convert2annovar.pl')
...
# table_annovar.pl example/ex1.avinput humandb/ -buildver hg19 -out myanno -remove -protocol refGene,esp6500siv2_all,1000g2015aug_all,avsnp150,ljb26_all,CLINSIG,exac03   -operation  g,f,f,f,f,f,f   -nastring . -csvout
    inputft= paras['inputfile_type']
    annovar_options=" "
    ...
    if not os.path.isfile(paras['table_annovar']):
        paras['table_annovar'] = os.path.join(os.path.dirname(os.path.realpath(__file__))+'/annovar', 'table_annovar.pl')
    if inputft.lower() == 'avinput' :
        cmd="perl "+paras['table_annovar']+" "+paras['inputfile']+" "+paras['database_locat']+" -buildver "+paras['buildver']+" -remove -out "+ paras['outfile']+" -protocol refGene,esp6500siv2_all,1000g2015aug_all,avsnp150,dbnsfp33a,clinvar_20170905,exac03,dbscsnv11,dbnsfp31a_interpro,rmsk,ensGene,knownGene  -operation  g,f,f,f,f,f,f,f,f,r,g,g   -nastring ."+annovar_options
        print("%s" %cmd)
        os.system(cmd)
    if inputft.lower() == 'vcf' :
...
def check_PS3(line,Funcanno_flgs,Allels_flgs, PS3_aachange_dict):
    '''
    ...
    PS3=0
    cls = line.split('\t')
    aa_change_info = cls[Funcanno_flgs['AAChange.refGene']]
    aa_change_list = re.findall('p\.(\w+\d+\w+)', aa_change_info)
    for elem in aa_change_list:
        keys_tmp2 = cls[Funcanno_flgs['Func.refGene']] + '_' + elem
        if keys_tmp2 in aa_changes_dict:
            PS3 = 1
            break
    return(PS3)
...
def check_PP5(line,Funcanno_flgs,Allels_flgs, PP5_aachange_dict):
    '''
    ...
    cls0 = re.split('[,;]', line_tmp2)
    for fc in funcs_tmp:
        if line_tmp2.find(fc) >= 0:
            for cls0_1 in cls0:
                aa = cls0_1[4]
                aa_last = aa[len(aa) - 1]
                aa_ref = aa[2]
                keys_tmp = cls[Allels_flgs['Chr']] + "_"+cls[Allels_flgs['Start']]+"_"+cls[Allels_flgs['End']]+"_"+ str(aa_ref) +"_"+ aa_last
                if keys_tmp in PP5_aachange_dict:
                    PP5 = 1
                    break
            break
    return(PP5)
...
def assign(BP,line,Freqs_flgs,Funcanno_flgs,Allels_flgs):
    ...
    PS[1]=PS2
    PS3=check_PS3(line,Funcanno_flgs,Allels_flgs, PS3_aachange_dict)
    PS[2]=PS3
    ...
    PP[3]=PP4
    PP5=check_PP5(line,Funcanno_flgs,Allels_flgs, PP5_aachange_dict)
    PP[4]=PP5
...
# ExAC_ALL esp6500siv2_all   1000g2015aug_all  SIFT_score    CADD_raw    CADD_phred  GERP++_RS   phyloP46way_placental  dbscSNV_ADA_SCORE   dbscSNV_RF_SCORE   Interpro_domain
    ...
    try:
        ...
        if re.findall('true',paras['otherinfo'], flags=re.IGNORECASE)  :
            fw.write("#%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % ("Chr","Start","End","Ref","Alt","Ref.Gene","Func.refGene","ExonicFunc.refGene", "Gene.ensGene","avsnp150","AAChange.ensGene","AAChange.refGene","Clinvar","InterVar and Evidence","Freq_ExAC_ALL", "Freq_esp6500siv2_all","Freq_1000g2015aug_all", "CADD_raw","CADD_phred","SIFT_score","GERP++_RS","phyloP46way_placental","dbscSNV_ADA_SCORE", "dbscSNV_RF_SCORE", "Interpro_domain","AAChange.knownGene","rmsk","MetaSVM_score","Freq_ExAC_POPs","OMIM","Phenotype_MIM","OrphaNumber","Orpha","Otherinfo"  ))
        else:
        ...
        for line in strs.split('\n'):
            ...
            if line_sum==0:
                ...
                if re.findall('true',paras['otherinfo'], flags=re.IGNORECASE)  :
                    fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (cls[Allels_flgs['Chr']],cls[Allels_flgs['Start']],cls[Allels_flgs['End']],cls[Allels_flgs['Ref']],cls[Allels_flgs['Alt']],cls[Funcanno_flgs['Gene']],cls[Funcanno_flgs['Func.refGene']],cls[Funcanno_flgs['ExonicFunc.refGene']], cls[Funcanno_flgs['Gene.ensGene']],cls[Funcanno_flgs['avsnp150']],cls[Funcanno_flgs['AAChange.ensGene']],cls[Funcanno_flgs['AAChange.refGene']],clinvar_bp,intervar_bp,cls[Freqs_flgs['ExAC_ALL']], cls[Freqs_flgs['esp6500siv2_all']], cls[Freqs_flgs['1000g2015aug_all']], cls[Funcanno_flgs['CADD_raw']],cls[Funcanno_flgs['CADD_phred']],cls[Funcanno_flgs['SIFT_score']],  cls[Funcanno_flgs['GERP++_RS']],".", cls[Funcanno_flgs['dbscSNV_ADA_SCORE']], cls[Funcanno_flgs['dbscSNV_RF_SCORE']], cls[Funcanno_flgs['Interpro_domain']],cls[Funcanno_flgs['AAChange.knownGene']],cls[Funcanno_flgs['rmsk']],cls[Funcanno_flgs['MetaSVM_score']],Freq_ExAC_POPs,OMIM,Pheno_MIM,orpha,orpha_details,cls[Funcanno_flgs['Otherinfo']]   ))
                else:
                    fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (cls[Allels_flgs['Chr']],cls[Allels_flgs['Start']],cls[Allels_flgs['End']],cls[Allels_flgs['Ref']],cls[Allels_flgs['Alt']],cls[Funcanno_flgs['Gene']],cls[Funcanno_flgs['Func.refGene']],cls[Funcanno_flgs['ExonicFunc.refGene']], cls[Funcanno_flgs['Gene.ensGene']],cls[Funcanno_flgs['avsnp150']],cls[Funcanno_flgs['AAChange.ensGene']],cls[Funcanno_flgs['AAChange.refGene']],clinvar_bp,intervar_bp,cls[Freqs_flgs['ExAC_ALL']], cls[Freqs_flgs['esp6500siv2_all']], cls[Freqs_flgs['1000g2015aug_all']], cls[Funcanno_flgs['CADD_raw']],cls[Funcanno_flgs['CADD_phred']],cls[Funcanno_flgs['SIFT_score']],  cls[Funcanno_flgs['GERP++_RS']],".", cls[Funcanno_flgs['dbscSNV_ADA_SCORE']], cls[Funcanno_flgs['dbscSNV_RF_SCORE']], cls[Funcanno_flgs['Interpro_domain']],cls[Funcanno_flgs['AAChange.knownGene']],cls[Funcanno_flgs['rmsk']],cls[Funcanno_flgs['MetaSVM_score']],Freq_ExAC_POPs,OMIM,Pheno_MIM,orpha,orpha_details  ))
...
#begin to process user's options:
    ...
    if options.database_intervar != None and os.path.exists(options.database_intervar):
        paras['database_intervar'] = options.database_intervar
    else:
        paras['database_intervar']=os.path.dirname(os.path.realpath(__file__)) + '/intervardb'
        ...
        paras['pp5_aa'] = paras['database_intervar'] + '/PP5.AA.change.patho'
        ...
        paras['ps3_aa'] = paras['database_intervar'] + '/PS3.AA.change.patho'
